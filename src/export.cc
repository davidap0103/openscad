...
// write out ASCII triangles to OBJ format. output stream must be open.
void ASCII_Triangles_to_obj( std::vector<std::string> &vertices, std::vector<triangle> &triangles, std::ostream &output ) {
	...
	for (size_t i = 0; i < triangles.size(); i++) {
		triangle t = triangles[i];
		output << "f ";
		output << find_index( vertices, t.vs1 ) << " ";
		output << find_index( vertices, t.vs2 ) << " ";
		output << find_index( vertices, t.vs3 ) << "\n";
	}
